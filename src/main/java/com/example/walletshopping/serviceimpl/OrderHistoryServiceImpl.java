...
public class OrderHistoryServiceImpl implements OrderHistoryService{
	...
	if (!cartProductList.isPresent()) {
	         ...
	         List<OrderHistoryDto> OrderList= cartProductList.get().stream().filter(orders-> (orders.getOrderDate().isEqual(LocalDate.now()))).map(orders -> getProductListDetails(orders)).collect(Collectors.toList());
	...
	if (!cartProductList.isPresent()) {
	         ...
	         List<OrderHistoryDto> OrderList= cartProductList.get().stream().filter(orders-> (orders.getOrderDate().getMonthValue() == month)).map(orders -> getProductListDetails(orders)).collect(Collectors.toList());
	         orderHistoryListDto.setMessage("please find list of orders");
	         orderHistoryListDto.setStatusCode(HttpStatus.OK.value());
	         orderHistoryListDto.setOrderHistoryDto(OrderList);
	            return orderHistoryListDto;
